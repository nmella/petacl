...
$updateConfigFunc = function (&$isEnabled, &$expiration, &$segmentableUseragents, &$segmentableCookies, &$isParameterSortingEnabled, &$ignoredParameters, &$isVirtualKeysEnabled, &$isAutoSwitcherEnabled) {
	global $autoloader;
	@clearstatcache();
...
$isEnabled = $expiration = $segmentableUseragents = $segmentableCookies = $isParameterSortingEnabled = $ignoredParameters = $isVirtualKeysEnabled = $isAutoSwitcherEnabled = null;
$updateConfigFunc($isEnabled, $expiration, $segmentableUseragents, $segmentableCookies, $isParameterSortingEnabled, $ignoredParameters, $isVirtualKeysEnabled, $isAutoSwitcherEnabled);
...
while ($line = fgets($handle)) {
	...
	if ($isAutoSwitcherEnabled === true) {
		// don't enable on first request
		if (isset($allCookies['frontend']) === false or isset($allParameters['__currency']) === true) {
			echo "\n"; continue;
		}
	}
